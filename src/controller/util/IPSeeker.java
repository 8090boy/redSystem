...
import controller.entity.IPEntry;
...
public class IPSeeker {
	...
	@Test
	public void testIp() {
		cityForIP("255.255.255.255");
	}

	public static String cityForIP(String ip) {
		if (ip.getBytes().length > 15) {
			System.out.println("=======不支持IPV6=======");
			return "null";
		}
		String address = null;
		List<String> strIPArr = new ArrayList<String>();
		strIPArr.add("127.0.0.1");
		strIPArr.add("255.255.255.255");
		strIPArr.add("0.0.0.0");

		for (String strIP : strIPArr) {
			if (ip.equals(strIP)) {
				return "null";
			}
		}
		...
		address = ips.getIPLocation(ip).getCountry();
		// System.out.println("===获取城市：====" + address + "======="); // 省市
		return address;
	}
	...
	public IPSeeker() {
		...
		try {
			...
			System.out.println("找不到QQWry.dat文件");
		}
		// 如果打开文件成功，读取文件头信息
		if (ipFile != null) {
			try {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (ipLoc.getCountry().indexOf(s) != -1 || ipLoc.getArea().indexOf(s) != -1) {
					...
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = IPSeekerTool.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = IPSeekerTool.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	public String getCountry(byte[] ip) {
		// 检查ip地址文件是否正常
		if (ipFile == null)
			return IPSeekerMessage.bad_ip_file;
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = IPSeekerTool.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
		return getCountry(IPSeekerTool.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
		return getArea(IPSeekerTool.getIpByteArrayFromString(ip));
	}
	...
	private IPLocation getIPLocation(byte[] ip) {
		...
		if (info == null) {
			info = new IPLocation();
			info.setCountry(IPSeekerMessage.unknown_country);
			info.setArea(IPSeekerMessage.unknown_area);
		}
		return info;
	}
	...
	private String readArea(long offset) throws IOException {
		...
		if (b == REDIRECT_MODE_1 || b == REDIRECT_MODE_2) {
			...
			if (areaOffset == 0)
				return IPSeekerMessage.unknown_area;
			else
	...
	private String readString(long offset) {
		try {
			...
			for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile.readByte())
				;
			if (i != 0)
				return IPSeekerTool.getString(buf, 0, i, "GBK");
		} catch (IOException e) {
			LogFactory.log("", Level.ERROR, e);
		}
		return "";
